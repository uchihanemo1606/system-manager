...
async function initHardwareDomainCreateModal(data) {
    ...
    const filterOS = document.querySelector("#filter-os");
    const filterDB = document.querySelector("#filter-db");
    const searchButton = document.querySelector("#search-hardware-btn");

    if (!table) return;

    table.innerHTML = `<tr><td colspan="5" class="text-center text-muted">Vui lòng nhập bộ lọc và nhấn "Tìm kiếm"</td></tr>`;

    searchButton.onclick = async () => {
        const ip = filterIP.value.trim();
        const db = filterDB.value.trim();
        const os = filterOS.value.trim();
        const deleted = filterDeleted.value;

        const params = new URLSearchParams();
        if (ip) params.append("ip", ip);
        if (db) params.append("dbname", db);
        if (os) params.append("OS", os);
        if (deleted !== "all") params.append("is_delete", deleted);
        ...
        try {
            const res = await get_all_hardware_connect_domain(params.toString());
            ...
            await Promise.all(
                hardwareList.map(hw =>
                    get_domain_by_hardware({ ip: hw.ip }).then(res => {
                        ...
                        if (domains.some(d => d.id == data.id)) {
                            linkedIPs.add(hw.ip);
                            selectedIPs.add(hw.ip);
                        }
                    })
                )
            );

            renderTable(hardwareList);
        } catch (err) {
            table.innerHTML = `<tr><td colspan="5" class="text-danger text-center">Lỗi khi tải dữ liệu.</td></tr>`;
            console.error(err);
        }
    };
    ...
    function renderTable(list) {
        if (!list.length) {
            table.innerHTML = `<tr><td colspan="5" class="text-center text-muted">Không có phần cứng nào.</td></tr>`;
            return;
        }

        table.innerHTML = list.map(hw => {
            const isChecked = selectedIPs.has(hw.ip);
            const checkedAttr = isChecked ? "checked" : "";
            const isDeleted = hw.is_delete;
            const rowClass = isDeleted ? "bg-light text-muted" : "";
        ...
        // Gán lại sự kiện checkbox và row click
        table.querySelectorAll(".hardware-row").forEach(row => {
            const cb = row.querySelector(".hw-checkbox");
            const ip = row.dataset.ip;
            ...
            row.addEventListener("click", e => {
                ...
                if (!cb.disabled) {
                    cb.checked = !cb.checked;
                    cb.checked ? selectedIPs.add(ip) : selectedIPs.delete(ip);
                }
            });
    ...
    // Nút liên kết
    linkButton.onclick = async () => {
        if (!selectedIPs.size && !linkedIPs.size) {
            return showToast({ message: "Chưa có phần cứng nào được chọn.", type: "warning" });
        }

        let successCreate = 0, failCreate = 0;
        let successRemove = 0, failRemove = 0;

        for (const ip of new Set([...selectedIPs, ...linkedIPs])) {
            const shouldBeLinked = selectedIPs.has(ip);
            const isLinked = linkedIPs.has(ip);
            ...
            if (shouldBeLinked && !isLinked) {
                try {
                    ...
                    console.error("Lỗi tạo domain:", ip, err);
                    failCreate++;
                }
            }
            ...
            if (!shouldBeLinked && isLinked) {
                try {
                    ...
                    console.error("Lỗi gỡ domain:", ip, err);
                    failRemove++;
                }
            }
        }
